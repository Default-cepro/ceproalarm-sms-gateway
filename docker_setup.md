# Docker Setup (Linux server)

Este flujo levanta el backend dentro de un contenedor en el servidor Linux. No mueve ADB al contenedor: ADB (forward/reverse) sigue corriendo en el host y el backend consume la API local del teléfono por `127.0.0.1:18080` usando `network_mode: host`.

`docker-compose.yml` ya fuerza:
- `SMS_GATE_LOCAL_API_ENABLED=1`
- `SMS_GATE_LOCAL_API_BASE_URL=http://127.0.0.1:18080`
- `network_mode: host` (ADB y la API del teléfono quedan alcanzables en `127.0.0.1:18080`)
...
Si tu usuario no es UID/GID 1000, exporta antes:
...
> Este setup apunta al servidor Linux con `network_mode: host` y ADB en el host.

## 3) ADB local (si aplica)

Ejecuta el script ADB en el host, **no** dentro del contenedor:
